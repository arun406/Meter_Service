...
public class RCSPlanVolumeGenerator implements Meter {
    ...
    public com.aktimetrics.core.transferobjects.Measurement measure(String tenant, String measurementCode, Step step) {
        ...
        return new com.aktimetrics.core.transferobjects.Measurement(tenant, mi.getId().toString(), step.getProcessInstanceId(),
                step.getId(), this.getStepCode(),
                measurementCode, String.valueOf((double) step.getMetadata().get("reservationVolume")),
                (String) step.getMetadata().get("reservationVolumeUnit"),
